...
def model(input_data):
	output_data = Lambda(
		process_img, input_shape=(160, 320, 3), output_shape=(83, 179, 2), name='process'
	)(input_data)

	output_data = Conv2D(
		24, 5, (2, 2), activation=relu, input_shape=(83, 179, 2), data_format='channels_last', name='conv1a'
	)(output_data)
	output_data = Conv2D(
		24, 3, (1, 1), activation=relu, input_shape=(40, 88, 24), data_format='channels_last', name='conv1b'
	)(output_data)
	output_data = MaxPooling2D(
		(3, 3), (2, 2), input_shape=(38, 86, 24), data_format='channels_last', name='pool1c'
	)(output_data)

	output_data = Conv2D(
		32, 3, (2, 2), activation=relu, input_shape=(19, 43, 24), data_format='channels_last', name='conv2a'
	)(output_data)
	output_data = Conv2D(
		32, 3, (1, 1), activation=relu, input_shape=(8, 20, 32), data_format='channels_last', name='conv2b'
	)(output_data)
	output_data = Conv2D(
		42, 3, (2, 2), activation=relu, input_shape=(6, 18, 32), data_format='channels_last', name='conv2c'
	)(output_data)
...
def train_model(model, train_ds, test_ds, save_to, quiet=True) -> Model:
	...
	if (EPOCHS <= WARMUP_EPOCHS) or (LEARNING_RATE_MIN > LEARNING_RATE_MAX):
		raise Exception
	global_steps = 1
	...
	for epoch in range(EPOCHS):
		...
		for images, labels in train_ds:
			...
			if not quiet:
				train_result(
					epoch, EPOCHS, batch_num, batches, metrics_loss.result(),
					metrics_steering_accuracy.result(), 0.0,
		...
		if not quiet:
			test_result(
				epoch, EPOCHS, metrics_loss.result(), metrics_steering_accuracy.result(), 0.0, timer.time_left())
